...
// (candidate.author ? '<p>Author: ' + candidate.author + '</p>':'')
    ...
    window.tests.forEach(function(test,j) {
      ...
      html += '<td data-candidate="' + i + '" data-test="' + j + '" class="result ' + (i == 0 ? ' expected-result' : passed ? 'passed' : 'failed') + '"><pre>' + toPrint(testResults[i][j]).replace(/ /g, '&nbsp;').replace(/\t/g, '[tab]') + '</pre></td>';
// .replace(/ /g, '&nbsp;')
//      html += '<td data-candidate="' + i + '" data-test="' + j + '" class="result ' + (passed ? 'passed' : 'failed') + '"></td>';
  ...
  $('td[data-candidate]').hover(
    function(e) {
      ...
      html = ''
      html += '<b>Test HTML:</b><br><pre>' + htmlEscapeEtc(test.html) + '</pre>';
      html += '<b>Test code:</b><br><pre>' + htmlEscapeEtc(test.code) + '</pre>';
//      html += '<b>jQuery result:</b><br><pre>' + toPrint(testResults[0][j]) + '</pre>';
//      html += '<b>This result:</b><br><pre>' + toPrint(testResults[i][j]) + '</pre>';
      html += '<b>jQuery result:</b> ' + toPrint(testResults[0][j]).replace(/ /g, '&nbsp;') + '<br><br>';
      html += '<b>This result:</b> ' + toPrint(testResults[i][j]).replace(/ /g, '&nbsp;') + '<br><br>';
...
//      $('#infobox').offset($(this).offset());
      $('#infobox').offset({top: e.pageY, left:e.pageX});
...
//      html += '<td>' + toPrint(result) + '</td>';
      ...
      var passed = (testResults[0][j] == testResults[i][j]);
      html += '<td class="expected-result">' + toPrint(testResults[0][j]) + '</td>';
      html += '<td class="result ' + (passed ? 'passed' : 'failed') + '">' + toPrint(testResults[i][j]) + '</td>';
