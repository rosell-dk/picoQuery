...
(function(w,d,u,z) {
  if (d.querySelectorAll && d.addEventListener) {
    if (!w.$) {
      ...
      // methods
      $.fn = P.prototype = {
        addClass:function(a){this.e.forEach(function(m){m.classList?m.classList.add(a):m.className+=" "+a});return this},
        append:function(){z=[].slice.call(arguments);this.e.forEach(function(m){z.forEach(function(a){$(a).e.forEach(function(c){m.appendChild(c)})})});return this},
        appendTo:function(a){this.e.forEach(function(el){$(a).append(el)});return this},
        css:function(a,b){u===b?this.e[0].style[a]:this.e.forEach(function(m){m.style[a]=b});return this},
        ...
        next:function(selector){return $(this.e.map(function(a){return a['nextElementSimling']}))},
        ...
        removeClass:function(a){this.e.forEach(function(m){m.className=m.className.replace(new RegExp("\b"+a.replace(" ","\b|\b")+"\b","g")," ")});return this},
        trigger:function(a){this.e.forEach(function(m){z=d.createEvent('HTMLEvents');z.initEvent(a,!0,!1);m.dispatchEvent(z)});return this}
      }
